...
class K_Data_60m_Model_Log_Dao:
    @exc_time
    def exists(self, code):
        # 绑定引擎
        # 连接数据表
        table = Table('k_data_60m_model_log', dataSource.mysql_quant_metadata, autoload=True)
    ...
    def insert(self, code, name, best_estimator, train_score, test_score, desc=None):
        sql = text('replace into k_data_60m_model_log (date, code, name, best_estimator, train_score, test_score, `desc`) '
...
k_data_60m_model_log_dao = K_Data_60m_Model_Log_Dao()
