...
def prepare_daily_data_from_db(code,start, end):
    sql = 'SELECT t1.code, t1.date,t1.open, t1.close, t1.high, t1.low, t1.volume' \
          ' from tick_data_daily t1' \
          ' where t1.code=\'%s\' and t1.date > \'%s\' and t1.date < \'%s\'' % ( code,start, end)
...
if __name__ == "__main__":
    #code = '600179'
    df = prepare_daily_data_from_db(code='000001',start='2017-01-01', end='2018-02-01')

    df_codes = ts.get_hs300s()

    for code in df_codes['code'].values:
        d = prepare_daily_data_from_db(code=code,start='2017-01-01', end='2018-02-01')
        df = df.append(d)
