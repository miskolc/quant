...
class KDJStrategy:
    ...
    def init(self, context):
        ...
        context.futu_quote_ctx = ft.OpenQuoteContext(host=default_config.FUTU_OPEND_HOST, port=default_config.FUTU_OPEND_PORT)

    @exc_time
    def handle_data(self, context):

        target_frame = pd.DataFrame(columns=['code', 'close', 'k_value', 'd_value', 'pre_k', 'pre_d', 'ma20', 'profits_yoy', 'bm'])
        for code in context.pool:
            daily_stock_data = k_data_dao.get_k_data(code=code, start=get_next_date(days=-30, args=context.current_date),
                                               end=get_current_date(context.current_date), futu_quote_ctx=context.futu_quote_ctx)
            ...
            if pre_k < pre_d and ((k_value >= d_value) or (abs(k_value - d_value)<=5)):
                ...
                if last_close > ma20_close and profits_yoy > 30:
                    target_stock = {'code': code, 'close': last_close, 'k_value': k_value, 'd_value': d_value, 'pre_k': pre_k, 'pre_d': pre_d, 'ma20': ma20_close, 'profits_yoy': profits_yoy, 'bm': 1/profits_yoy}
                    target_frame.concat([target_frame, target_stock])
